...
#if defined(NDEBUG)
# error "Globalbestcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x95f5d3931da8c9458a1264d7c8b01d98e7e395959a5559a28794e9768a32ef61");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Globalbestcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Globalbestcoin Signed Message:\n";
...
{
    // Globalbestcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight > 1000)  
    {
        nSubsidy = 2 * COIN;  // premine on 3rd block
    }
	if(nHeight > 3000)  
    {
    ...
    nSubsidy >>= (nHeight / 840000); // Globalbestcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Globalbestcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Globalbestcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Globalbestcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Globalbestcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x231b55c94b492ae44e4dedb02820692ad9a1b216759672944b2f61139d7d9301");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BayueWalker Very Rich from this Year 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1537255824;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087142974;

        if (fTestNet)
        {
            block.nTime    = 1537255839;
            block.nNonce   = 387687185;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x366032fe34924e7cd49ef11bd9469ba75f01827b27c6c8bd916b1e3936bc3cda"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc2, 0xbd, 0xda }; // Globalbestcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlobalbestcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlobalbestcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlobalbestcoinMiner(CWallet *pwallet)
{
    printf("GlobalbestcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("globalbestcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GlobalbestcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlobalbestcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlobalbestcoinMiner, pwallet));
}
